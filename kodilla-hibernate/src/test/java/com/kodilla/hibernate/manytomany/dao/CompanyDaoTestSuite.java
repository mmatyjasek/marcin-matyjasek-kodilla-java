...
public class CompanyDaoTestSuite {
    ...
    public void testNamedQueries() {
        ...
        int softwareMakersId = softwareMakers.getId();
        companyDao.save(softDev);
        int softDevId = softDev.getId();

        //When
        List<Employee> employeesWithLastName = employeeDao.retrieveEmployeeByLastName("Kovalsky");

        List<Company> companiesWithNameStartingWith = companyDao.retrieveCompaniesWithNameStartingWith("Sof%");

        //Then
        try {
            Assert.assertEquals(2, employeesWithLastName.size());
            Assert.assertEquals(3, companiesWithNameStartingWith.size());
            ...
            try {
                ...
                companyDao.deleteById(dataMaestersId);
                companyDao.deleteById(greyMatterId);
                companyDao.deleteById(softwareMakersId);
                companyDao.deleteById(softDevId);
            } catch (Exception e) {
                //do nothing
            }
        }
